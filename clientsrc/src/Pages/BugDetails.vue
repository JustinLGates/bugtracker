      ...
      <div class="row p-1 pt-3">
        <div class="col-12 d-flex justify-content-between bg-light p-0 rounded-top">
          <div class="p-1 pt-3 m-2">
            ...
            <div v-if="this.profile.email == this.bugDetails.creatorEmail && this.editing == true">
              <h2 class="action" @click.prevent="toggleEdit">{{editBugForm.title}}</h2>
              <input class="p-2 rounded" v-model="editBugForm.title" type="text" />
            </div>
            <h2
              @click.prevent="toggleEdit"
              v-else
              class="text-dark p-0 m-0 action"
            >{{this.bugDetails.title}}</h2>
          </div>
        </div>
          ...
          <div class>
            ...
            <form @submit.prevent="editBug">
              ...
              <p
                @click.prevent="toggleEdit"
                v-else
                class="px-1 m-0 action"
              >{{this.bugDetails.description}}</p>
            </form>
...
export default {
  ...
  methods: {
    toggleEdit() {
      ...
      this.editBugForm.title = this.bugDetails.title;
      this.editBugForm.description = this.bugDetails.description;
    },
    closeBug() {
